...
export default function Banner({ options }) {
  ...
  return (
    <div className="w-full overflow-x-clip relative">
      ...
      <div
        className={`flex items-center absolute top-0 w-[120%] h-[100px] bg-black text-white dark:bg-white dark:text-black -rotate-[4deg] -translate-x-4 z-10`}
      >
        {content.map((item, index) => {
          if (item === 'star') {
            ...
            return (
              <div
                key={index}
                className="text-3xl mx-2 whitespace-nowrap font-main"
              >
                {item}
              </div>
            );
